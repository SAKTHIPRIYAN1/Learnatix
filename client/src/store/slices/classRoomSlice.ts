...
const initialState: initialStateTyp = {
  activeTab: "chat",
  chatMessages: [
    { senderName:"sakthi", message: "Hey, how are you doing?", senderId: "user_31myhYnbFuTvBaa7P0aY7DAXXtc" },
    { senderName:"sakthi", message: "I’m good, thanks! What about you?", senderId: "user2" },
    { senderName:"sakthi", message: "Pretty busy with the project, but it’s going well.", senderId: "user_31myhYnbFuTvBaa7P0aY7DAXXtc" },
    { senderName:"sakthi", message: "That’s nice bjkbdkfbgkdbgbbdjkbgjksdbgjkbdkgbbdjbjkdbgjkdbfkgsdbjjsdkgdjksggjkdfjkbdbjk djkgfjsdk gsd fgh kghfdhjkh  hfdh i want to acheive the nav system (in side page.tsx at top) like drive and windows file system eg: parfolder->subFolder->curFolder when i click it it must navigate to that route , how can i acheive like production ready app use generall route in redux centerailized state or for each pages.tsx maunually set that in next js app routeri want to acheive the nav system (in side page.tsx at top) like drive and windows file system eg: parfolder->subFolder->curFolder when i click it it must navigate to that route , how can i acheive like production ready app use generall route in redux centerailized state or for each pages.tsx maunually set that in next js app routeri want to acheive the nav system (in side page.tsx at top) like drive and windows file system eg: parfolder->subFolder->curFolder when i click it it must navigate to that route , how can i acheive like production ready app use generall route in redux centerailized state or for each pages.tsx maunually set that in next js app router to hear. Need any help?", senderId: "user2" },
    { senderName:"sakthi", message: "Maybe later. For now, I’m testing the chat UI.", senderId: "user2" },
    { senderName:"sakthi", message: "Alright, ping me if anything comes up.", senderId: "user_31myhYnbFuTvBaa7P0aY7DAXXtc" },
  ],
};
...
export const {setActiveTab} =ClassRoomSlice.actions;
